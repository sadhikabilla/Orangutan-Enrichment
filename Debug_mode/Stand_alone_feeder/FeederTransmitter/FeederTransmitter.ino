/*Sends signal to the feeder receiver. If the value is above the treshold (both the wires being touched)
a turn 90 message is sent and turn 0 otherwise.*/ 
...
if((value0 > (movingAverage0 + capThresholdval))  && (value1 > (movingAverage1 + capThresholdval)) ){
    ...
    //If value above threshold, turn the motor
    transmitter.send("TURN 90");
    Serial.println("TURN 0");
       
  }else {
   
    //If value below threshold, do not turn the motor
